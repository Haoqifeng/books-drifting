...
const verifyBook = async (bookTitle, bookId) => {
  try {
    ...
    // 获取当前用户的借阅记录
    const response = await axios.get(`https://books-drifting-production.up.railway.app/api/borrow/current/user/${userId}`)
...
const confirmReturnBook = async () => {
  ...
  try {
    const response = await axios.post(`https://books-drifting-production.up.railway.app/api/borrow/return/${bookInfo.value.id}`)
